...
function rollbackfromcache() {//set the input number cache and rollback after cancel 2017-12-01 Kenny
    jQuery("#row1 input").val(cache_tzCount);
	jQuery("#row2 input").val(cache_nsaCount);
	jQuery("#row3 input").val(cache_eclassCount);
	jQuery("#row4 input").val(cache_eclassMidCount);
	jQuery("#row5 input").val(cache_eclassHighCount);
	jQuery("#row6 input").val(cache_userCount);
	jQuery("#row7 input").val(cache_hoursPerUser);
...
function clickClose() {
    jQuery('#calculateSyslogDiv').foundation('toggle');
}

/*function updateSyslogs(){//20171108 Kenny
	clickClose();
	var tzCount = parseInt(document.calculateSyslog.tzCount.value);
	var nsaCount = parseInt(document.calculateSyslog.nsaCount.value);
	var eclassCount = parseInt(document.calculateSyslog.eclassCount.value);
	var eclassMidCount = parseInt(document.calculateSyslog.eclassMidCount.value);
	var eclassHighCount = parseInt(document.calculateSyslog.eclassHighCount.value);
	var userCount = parseInt(document.calculateSyslog.userCount.value);
	var hoursPerUser = parseInt(document.calculateSyslog.hoursPerUser.value);
	if (mode==0)
		document.calc.syslogs.value = (userCount*hoursPerUser*syslogsPerUserPerHour).toFixed(3);
	else
		document.calc.syslogs.value = (tzCount*syslogsPerTz+nsaCount*syslogsPerNsa+eclassCount*syslogsPerEclass+eclassMidCount*syslogsPerEclassMid+eclassHighCount*syslogsPerEclassHigh).toFixed(3);
	calculate();
}*/
...
function updateSyslogs() {
	if(validateInput())
	{
		...
		//jQuery('a.remove-mm').trigger('click');
		//jQuery('#calculateSyslogDiv').foundation('reveal', 'open');
	}
...
function calculate() {
	...
	document.calc.summarizers.value = "1 AIOP/Console" + (multipleAgents? ", " + value + " Agents": "");
	...
	document.calc.database.value = addCommas((dbSize+filesSize).toFixed(3));
	document.calc.cache.value = addCommas(cacheSize.toFixed(3));
	document.calc.totaldisk.value = addCommas((dbSize+filesSize+backupSize + cacheSize).toFixed(3));
}
