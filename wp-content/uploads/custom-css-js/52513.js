...
function calculate() {
	...
	//Deal with Agents Server numbers 2019-04-16 Kenny
	//agents = value;	
	//if (document.calculateSyslog.ReportType[1].checked) {//flow based
		//agents = agents/3;
	//}
	
	//if (agents < 1) agents = 1;
	//else agents = parseInt(agents) + 1;
	...
	totalDisk = dbSize+filesSize+backupSize + cacheSize;
	dbValue = dbSize+filesSize;
	...
	if (document.calculateSyslog.ReportType[1].checked) {//flow based
		...
		if (dbValue < 1) dbValue = 1;
			...
			if( parseInt(dbValue) < dbValue) dbValue = parseInt(dbValue) + 1;
			else dbValue = parseInt(dbValue);
		...
		if (cacheSize < 1) cacheSize = 1;
			...
			if(parseInt(cacheSize) <cacheSize ) cacheSize = parseInt(cacheSize) + 1;
			else cacheSize = parseInt(cacheSize);
		}
		totalDisk = totalDisk/2;
		if (totalDisk < 1) totalDisk = 1;
		else {
			if(parseInt(totalDisk) < totalDisk) totalDisk = parseInt(totalDisk) + 1;
			else totalDisk = parseInt(totalDisk);
			
		}
	}
	
	document.calc_output.database.value = addCommas((dbValue).toFixed(3));
	document.calc_output.cache.value = addCommas(cacheSize.toFixed(3));
	document.calc_output.totaldisk.value = addCommas((totalDisk).toFixed(3));
}
