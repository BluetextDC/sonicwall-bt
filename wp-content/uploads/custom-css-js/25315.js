    ...
    jQuery(document).on('gform_post_render', function() {
        ...
        jQuery.ajax({
            url: '/wp-content/uploads/xml/sonicwall-upgrade-trade-in-v2.xml',
            dataType: 'xml',
            beforeSend: function() {
        ...
        jQuery('.newUpgradeBrand').on('change', function() {
            disableRegForm();
            jQuery('.newUpgradeBrand').prop('disabled', false);
            jQuery('.newUpgradeAvailability').attr("disabled", true).children().not(":first").remove();
            jQuery('#optionalDescription').html('');
        ...
        jQuery('.newUpgradeAvailability').on('change', function() {
            if (jQuery(this).val() == '') {
                ...
                var productName = jQuery(this).val().replace(/\s/g, ''),
                    description = '',
                    sku = ' ';
                ...
                jQuery(pricelistdescription).find("Products").find('Product').each(function() {
                    ...
                    if (productXMLName == productName || productXMLName.indexOf(productName) !== -1) {
                        ...
                        //jQuery('#input_44_122').val('Product Type: ' + jQuery('.newUpgradeType').val() + " - Currently Owned: " + jQuery('.newUpgradeBrand').val() + " - Selected Upgrade: " + jQuery('.newUpgradeAvailability').val());
        ...
        if (target == '.newUpgradeBrand') {
            jQuery(dataset).find('Type').each(function() {
                if (jQuery(this)[0].getAttribute('name') == product) {
                    ...
                    jQuery(this).find('Brand').each(function() {

                        if (jQuery(this).find("Model").length > 0) { //if we have models, then print brand and then models as optgroups

                            jQuery(target).append('<optgroup label="' + jQuery(this).attr('name') + '">'); //print the brand

                            jQuery(this).find("Model").each(function() {
                                jQuery(target).append('<option value="' + jQuery(this).attr('name') + '">' + jQuery(this).attr('name') + '</option>'); //print each model under the brand
                            });
                        } else { //otherwise, print brands as the options for email clients

                            jQuery(this).each(function() {
                                jQuery(target).append('<option value="' + jQuery(this).attr('name') + '">' + jQuery(this).attr('name') + '</option>');
                            });
                        }
            ...
            if (jQuery('.newUpgradeType').val() == "EMS") { //handle EMS differently
                jQuery(dataset).find('Type').each(function() {
                    if (jQuery(this).attr('name') == "EMS") {

                        jQuery(this).find('Brand').each(function() {
                            if (jQuery(this).attr('name') == product) {

                                jQuery(this).find('Product').each(function() {
                                    jQuery(target).append('<option value="' + jQuery(this).text() + '">' + jQuery(this).text() + '</option>');
                                    show = true;
                                });
                ...
                jQuery(dataset).find('Model').each(function() {
                    if (jQuery(this).attr('name') == product) {
