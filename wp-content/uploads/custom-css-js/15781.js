...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        jQuery('Choose Your Type').insertBefore('.upgrade-type');

        var upgradeBrandName = jQuery('.upgrade-brand').attr('name');
        jQuery('.upgrade-brand').attr('name','upgradeBrand').hide();;
        jQuery('Choose Your Brand & Model').insertBefore('.upgrade-brand');
        ...
        jQuery('Available SonicWall Upgrades').insertBefore('.upgrade-availability');
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                jQuery(dataset).find('Types').children().each(function () {
                    jQuery('.newUpgradeType').append('' + jQuery(this)[0].getAttribute('name') + '');
                });
            }
        });
            ...
            if (jQuery(this).val() == '') {
                ...
                jQuery(pricelistdescription).find("Products").find('Product').each(function () {
                    ...
                    if (productXMLName == productName || productXMLName.indexOf(productName) !== -1) {
						...
						jQuery('#input_9_96').val('Product Type: ' + jQuery('.newUpgradeType').val() + " - Currently Owned: " + jQuery('.newUpgradeBrand').val() + " - Selected Upgrade: " + jQuery('.newUpgradeAvailability').val());
                        return false;
                    }
                });
        ...
        jQuery('#banner-link').append('<span class="">\
	  <span class="headline">Get a great deal on deeper <br />network security </span>\
	  <span class="subheadline">Join the Dell Security Customer Advantage Program.</span>\
	  <span class="action text">Read Data Sheet<span>›</span></span>\
	</span>');
    });
            ...
            jQuery(dataset).find('Type').each(function () {
                if (jQuery(this)[0].getAttribute('name') == product) {
                    ...
                    jQuery(this).find('Product').each(function () {
                         jQuery(target).append('' + jQuery(this).attr('name') + ''); //print each model under the brand
                    });

//                    jQuery(this).find('Product').each(function () {
//
//
//                            jQuery(target).append(''); //print the brand
//
//                            jQuery(this).find("UpgradeProduct").each(function () {
//                                jQuery(target).append('' + jQuery(this).text() + ''); //print each model under the brand
//                            });
//
//                        show = true;
//
...
jQuery(document).on('gform_post_render',function() {
	...
	jQuery("#input_45_14").addClass('crs-country');
	jQuery("#input_45_132").addClass('crs-state');
    ...
    jQuery("select.crs-country").attr("data-region-id", "crs-state").attr("data-value", "shortcode").attr("data-default-option", "Select Country").attr("data-show-default-option", "true");

    //	Set State selector field attributes as required by the script.  By default this is disabled and says N/A
    jQuery("select.crs-state").attr("data-blank-option", "N/A").attr("data-value", "shortcode").attr("data-default-option", "N/A").attr("data-show-default-option", "true").prop("disabled", true).attr("title", "You have not selected a country yet, or your selection does not require a state entry.").removeClass("required");
    ...
    jQuery('select.crs-country').change(function() {
        ...
        if(jQuery(this).val() == "US"){
            ...
            // jQuery('.canada-optin input').rules('add','required');
        }
        else {
            //jQuery('.canada-optin').hide();
            //jQuery('.canada-optin').show();
            //jQuery('.canada-optin input').rules('remove','required');
        }
