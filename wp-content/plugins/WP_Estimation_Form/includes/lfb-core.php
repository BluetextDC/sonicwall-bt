...
class LFB_Core {
        ...
        if ($chkDisplay) {
            ...
            if ($dataItem->type == 'picture') {
                ...
                if ($dataItem->tooltipText != "") {
                    $dataTooltip = 'data-toggle="tooltip"  data-placement="bottom" data-tooltiptext="' . $dataItem->tooltipText . '"';
                }
                ...
                if ($dataItem->callNextStep) {
                 ...
                 $response .= '<img  class="lfb_imageButtonImg ' . $svgClass . '" src="'.$dataItem->image.'" data-placement="bottom" title="' . $dataItem->title . '" data-originaltitle="' . $originalTitle . '"  alt="' . $dataItem->imageDes . '" />';
                ...
                $response .= '<div class="form-group">';
                $response .= '<label>' . $dataItem->title . '</label>
                ' . $conditionalWrapStart . '<input data-urlvariable="' . $dataItem->sendAsUrlVariable . '" data-variablename="' . $dataItem->variableName . '" type="text" ' . $dataTooltip . ' data-mintime="' . $dataItem->minTime . '"  placeholder="' . $dataItem->placeholder . '" data-maxtime="' . $dataItem->maxTime . '" data-itemid="' . $dataItem->id . '"  ' . $showInSummary . '  ' . $hideQtSummary . '   ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '  class="form-control lfb_timepicker" ' . $itemRequired . ' data-title="' . $dataItem->title . '" data-originaltitle="' . $originalTitle . '"  ' . $urlTag . '/>' . $conditionalWrapEnd;
        ...
        if (!$form->useCaptcha || ($_SESSION['lfb_random_number'] != "" && strtolower($_SESSION['lfb_random_number']) == strtolower($_POST['captcha']))) {
            ...
            if (!isset($_POST['gravity']) || $_POST['gravity'] == 0) {
                ...
                if ($_POST['email_toUser'] == '1') {
                    ...
                    $content = str_replace("[total_price]", stripslashes(sanitize_text_field($_POST['totalTxt'])), $content);
    ...
    public function uploadFormFiles() {
        ...
        $table_name = $wpdb->prefix . "wpefc_forms";
        $formReq = $wpdb->get_results($wpdb->prepare("SELECT id,randomSeed FROM $table_name WHERE id='%s' LIMIT 1", $formID));
        if (count($formReq) > 0) {
        $form = $formReq[0];
