...
function lfb_getUrlVariables(formID, items) {
    ...
    if(variablesText.length>0){
        variablesText = variablesText.replace(/ /g, "");
...
function wpe_previousStep(formID) {
    ...
    if(jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel .genSlide[data-stepid="' + lastStepID + '"]').length>0){
     ...
     jQuery('#estimation_popup.wpe_bootstraped[data-form="' + formID + '"] #mainPanel .genSlide[data-stepid="' + lastStepID + '"]').css('opacity',0);
    }
...
function lfb_executeCalculation(calculation, formID, targetID) {
    ...
    while ((i = calculation.indexOf('item-', i + 1)) != -1) {
        ...
        if (action == 'quantity') {
            ...
            //value = wpe_formatPrice(value, formID);
        }
        if (action == 'value') {
