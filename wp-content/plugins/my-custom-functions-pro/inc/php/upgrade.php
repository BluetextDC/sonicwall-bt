...
/**
 * Upgrade plugin data in the database to version 2.0
 */
function spacexchimp_p011_upgrade_2_0() {

    // Upgrade number
    $upgrade_number_new = '0001';
    $prefix_old = 'MCFunctionsPRO';
    $prefix_new = 'spacexchimp_p011';
    ...
    $settings_current = get_option( $prefix_old );
    $snippets_old = !empty( $settings_current['functions'] ) ? $settings_current['functions'] : '';

    // Setting array with new data
    $settings_new = is_array( $settings_current ) ? $settings_current : array();
    $settings_new['snippets'] = $snippets_old;
    unset($settings_new['functions']);
    ...
    delete_option( $prefix_old );
    update_option( $prefix_new . '_settings', $settings_new );

}
spacexchimp_p011_upgrade_2_0();
