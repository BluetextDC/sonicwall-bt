jQuery(function ($) {
	...
	$(document).ready(function () {
		...
		$('.video-holder').on("mouseover", function (e) { // added to resolve overlay hover issue www2-1583
			$('.video-holder a span.image-overlay').css("top", "0");
		});
		...
		if ($('#video-container').length > 0) {
			...
			$("#video-taxonomy-form-holder #grid-video-page-num").val(curpg);
		}

		$("#video-container .page-numbers .video-prev-page").on("click", function() {
			const prev = Number($("#video-taxonomy-form-holder #grid-video-page-num").val()) - 1;
			$("#video-taxonomy-form-holder #grid-video-page-num").val(prev);
			video_form_sumbit();
		});
		$("#video-container .page-numbers .video-next-page").on("click", function() {
			const next = Number($("#video-taxonomy-form-holder #grid-video-page-num").val()) + 1;
			$("#video-taxonomy-form-holder #grid-video-page-num").val(next);
			video_form_sumbit();
		});
		...
		let vid_srch = $('#cur-vid-search').html();
		if (vid_srch != '' && vid_srch != undefined) {
			vid_srch = vid_srch.replace(/\+/g, ' ');
			$('#grid-video-search').val(vid_srch);
		}

		$('#video-taxonomy-selector').on('change', function (e) {
			$("#grid-video-page-num").val(1);
			video_form_sumbit();
		});
		$('#video-taxonomy-form .search-btn').on("click", function (e) {
			$("#grid-video-page-num").val(1);
			video_form_sumbit();
		});
		function video_form_sumbit() {
			if($("#grid-video-search").val() === '') {
				$("#grid-video-search").prop('disabled', true);
			}
			if($("#video-taxonomy-selector").val() === '') {
				$("#video-taxonomy-selector").prop('disabled', true);
			}
			if(Number($("#grid-video-page-num").val()) <= 1) {
				$("#grid-video-page-num").prop('disabled', true);
			}
			$('#video-taxonomy-form').submit();
		}

		if($("#single-video-container").length > 0) {
			chgTitle = $("#single-video-container #video-info-container h2").get(0).innerHTML;
			$("head title").get(0).innerHTML = chgTitle;
		}
