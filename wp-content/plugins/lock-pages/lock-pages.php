	...
	class SLT_LockPages {
		...
		*/
		function __construct() {
			...
			//Lock all posts
			add_action( 'wp_ajax_lock_all_posts',  array( &$this, 'lock_all') );
			add_action( 'wp_ajax_unlock_all_posts',  array( &$this, 'unlock_all') );
			add_action( 'wp_ajax_unlock_post_type', array( &$this, 'unlock_post_type') );
			add_action( 'wp_ajax_lock_post_type', array( &$this, 'lock_post_type') );
			add_action( 'wp_ajax_lock_post_id', array( &$this, 'lock_post_id') );
			add_action( 'wp_ajax_unlock_post_id', array( &$this, 'unlock_post_id') );
			add_action( 'wp_ajax_lock_english_posts', array( &$this, 'lock_english_posts' ) );
		}
		...
		*/
		function add_hooks() {
			...
			add_filter( 'wp_insert_post_data', array( &$this, 'prevent_all'), 0, 2 );
		...
		*/
		function output_page_locked_notice() {
			if ( $this->is_page_locked_for_current_user() ) {
				...
				echo '<script>window.page_is_locked = true;</script>';
		...
		function output_meta_box() {
			if ( current_user_can( $this->options[$this->prefix.'capability'] ) ) {
				...
				<label for="<?php echo esc_attr( $this->prefix ); ?>locked">
					<input class="lock_toggle" type="checkbox" name="<?php echo esc_attr( $this->prefix ); ?>locked" id="<?php echo $this->prefix; ?>locked"<?php if ( $this->is_page_locked( $post->ID ) ) echo ' checked="checked"'; ?> value="true" />
					<?php
					if ( get_post_type() == 'page' ) {
		...
		function lock_post_id()
		{
			...
			{
				die("post_id not defined");
			}
		}

		function unlock_post_id()
		{
			if (isset($_REQUEST['post_id']))
			{
				$post_id = $_REQUEST['post_id'];
		...
		function unlock_post_type()
		{
			if (isset($_REQUEST['post_type']))
			{
				...
				$posts = $wpdb->get_results(
					$wpdb->prepare($querystr, $post_type)
				, OBJECT);
			...
			{
				die("post_type not defined");
			}
			
		}

		function lock_english_posts()
		{
			global $wpdb;
			...
			$querystr = "
				SELECT $wpdb->posts.* 
				FROM $wpdb->posts p 
				JOIN wp_multilingual_linked ml 
					ON ml.ml_source_elementid = p.ID AND ml.ml_type='post';
			 ";
		...
		function prevent_all($data, $post)
		{
			$post_id = $post["ID"];

			if ( $post_id && !$this->user_can_edit( $post_id ) )
			{
				echo "<h2 style='color: red; text-align: center;'>Editing is disabled for this post.</h2>";
				echo '<button style="display: block; margin: 0 auto;" onclick="javascript:window.history.back();">Go Back</button>';
				die();
			}
		...
		*/
		function admin_menu_link() {
			...
			wp_enqueue_script( $this->prefix . '_js', plugins_url( 'lock-pages-admin.js', __FILE__ ), array( 'jquery' ), '3.0' );
		}
			...
			<div class="wrap">
				...
				<form method="post" id="SLT_LockPages_options">
					...
					<table width="100%" cellspacing="2" cellpadding="5" class="form-table">
						...
						<tr valign="top">
							<th width="33%" scope="row">Bulk Actions</th>
							<td>
								<button id="lock_all_posts_btn">Lock all posts</button>
								<hr />
								<button id="unlock_all_posts_btn">Unlock all posts</button>
								<hr />
								<button id="lock_all_english_posts_btn">Lock all English posts</button>
								<hr />
								<label>Lock specific post type:</label>
								<br />
								<select id="lock_post_type_select">
									<?php 
									foreach ( $post_types as $post_type ) {
								...
								<button id="lock_post_type_btn">Lock specific post type</button>
								<hr />
								<label>Unlock specific post type:</label>
								<br />
								<select id="unlock_post_type_select">
									<?php 
									foreach ( $post_types as $post_type ) {
										echo '<option value="' . $post_type . '">' . $post_type . '</option>';
									}
								...
								<button id="unlock_post_type_btn">Unlock specific post type</button>
								<hr />
							</td>
						</tr>
