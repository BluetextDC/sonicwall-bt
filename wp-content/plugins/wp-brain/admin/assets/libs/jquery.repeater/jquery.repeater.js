// jquery.input version 0.0.1 (modified)
...
(function ($) {
    ...
    var queryjs = (function () {
        ...
        var stringify = function (parsed) {
            ...
            return parsed.url + (parameterStrings.length > 0 ? '?' + parameterStrings.join('&') : '') + (parsed.hash ? '#' + parsed.hash : '');
        };
    ...
    var $getAnyForminatorModule = function (preSelector, name, moduleName) {
        return $(preSelector + (moduleName ? '-' + moduleName : '') + (name ? '-' + name : ''));
    };
...
// Fix label "for" in repeated items
(function ($) {
    'use strict';
    
    $.fn.fixRepeaterLabels = function(){
        this.find('table.settings-table th > label[for]').each(function(){
            var $label = $(this);
            if( $label.attr('for') == '' ) {
                var $input = $label.closest('tr').find('td .data-field');
                if( $input.length ) {
                    var input_id = $input.attr('id');
                    if( !input_id ) {
                        input_id = 'field_'+wpbrain.uniqid();
...
(function ($) {
    ...
    $.fn.repeater = function (fig) {
        ...
        $(this).each(function () {
            ...
            var $itemTemplate = $($self.find('script[data-repeater-template]').html());
            if(!$itemTemplate.length ) {
                $itemTemplate = $list.find('[data-repeater-item]').first().clone().hide();
            }
            ...
            var setIndexes = function ($items, groupName, repeaters) {
                $items.each(function (index) {
                    ...
                    $filterNested($item.find('[name]'), repeaters).each(function () {
                        var $input = $(this);
                        if( $input.closest('[data-repeater-noindex]').length ) {
                            return;
                        }
                        var matches = $input.attr('name').match(/\[[^\]]+\]/g);
                        var name = matches ? last(matches).replace(/\[|\]/g, '') : $input.attr('name');
