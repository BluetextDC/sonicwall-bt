...
class CustomCSSandJS_Revisions {
    ...
    function ajax_revisions_delete() {
        ...
        foreach ( $revisions as $_id ) {
            ...
            if ( !wp_is_post_revision( $_id ) ) continue;
            if ( wp_is_post_autosave( $_id ) ) continue;
            if ( !current_user_can( 'delete_post', get_post( $_id )->post_parent ) ) continue;
        ...
        $ajax = new WP_Ajax_Response( array('data' => 1, 'supplemental' => array( 'ids' => implode(',', $deleted ) ) ) );
        $ajax->send();
        return;
    }
    ...
    function ajax_revisions_compare() {
        ...
        if ( ! $options ) {
            $options = get_metadata( 'post', $post->ID, 'options', true );
        }


        switch( $options['language'] ) {
            case 'js' : $code_mirror_mode = 'text/javascript'; break;
            case 'html' : $code_mirror_mode = 'application/x-httpd-php'; break;
            default : $code_mirror_mode = 'text/css'; break;
        }
    ...
    function revisions_meta_table( $left, $right, $post ) {
        ...
        if ( $left->post_type == 'revision' ) {
            ...
            $options_left = get_post_meta( $left->ID);
            $options_left = unserialize( $options_left['options'][0] );
        }
        if ( isset( $options_left['language'] ) ) $language = $options_left['language'];

        $options_right = false;
        if ( $right->post_type == 'revision' ) {
            $options_right = get_metadata( 'post', $right->ID, 'options', true );
        } elseif ( $right->post_type == 'custom-css-js' ) {
            $options_right = get_post_meta( $right->ID);
            $options_right = unserialize( $options_right['options'][0] );
        }
    ...
    function admin_enqueue_scripts( $hook ) {
        ...
        if ( $screen->post_type != 'custom-css-js' ) return false;
...
<?php
    ...
    function wp_save_post_revision_post_has_changed( $post_has_changed, $last_revision, $post ) {
        ...
        if ( ! isset( $old_options['options'] ) ) {
            $old_options = false;
        } else {
            $old_options = $old_options['options'];
        }
        $new_options = get_post_meta( $last_revision->ID );
        if ( ! isset( $new_options['options'] ) ) {
            $new_options = false;
        } else {
