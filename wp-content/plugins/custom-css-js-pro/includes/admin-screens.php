...
class CustomCSSandJS_AdminPro {
    ...
    public function admin_enqueue_scripts( $hook ) {
        ...
        if ( $hook == 'custom-css-js_page_custom-css-js-config' ) {
            wp_enqueue_script( 'ccj-bootstrap', $a .'/bootstrap.3.2.0.min.js', array( 'jquery' ), $v, true  );
        ...
        if ( $hook == 'post-new.php' || $hook == 'post.php' ) {
            ...
            wp_enqueue_script( 'ccj-lints-js', $cma . 'lint-vendors/jshint.min.js', array('ccj-codemirror'), $v, false );
        ...
        if ( isset($this->settings['ccj_editor_theme']) && !empty($this->settings['ccj_editor_theme'] ) ) {
...
<!-- div class="button-right" id="ccj-search-button" alt="Search"><button role="presentation" type="button" tabindex="-1"><i class="ccj-i-find"></i></button></div -->
                ...
                <div class="code-mirror-before <?php if (!empty($editor_theme) ) echo 'cm-before-' . $editor_theme; ?>"><div><?php echo htmlentities( $code_mirror_before );?></div></div>
                ...
                <div class="code-mirror-after <?php if (!empty($editor_theme) ) echo 'cm-after-' . $editor_theme; ?>"><div><?php echo htmlentities( $code_mirror_after );?></div></div>
    ...
    function options_save_meta_box_data( $post_id ) {
        ...
        if ( $is_preprocessor ) {
            $options['preid'] = trim($_POST['custom_code_preid']);
    ...
    function render_input( $_key, $a, $options ) {
        ...
        if ( $a['type'] === 'radio' ) {
            ...
            if ( $_key === 'preprocessor' && ( $options['preprocessor'] === 'less' || $options['preprocessor'] === 'sass' ) ) {
                ...
                $preid_info_text = __('This ID can be used in a Less/SASS import statement from another custom CSS code. Example: if this ID is \'vars.scss\', then you can write <b>@import \'vars.scss\';</b> in another custom Less/SASS code in order to import it. Leave the ID empty if it will not be imported from another custom code.');
