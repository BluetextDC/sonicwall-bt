...
class TimelineExpressBase {
	...
	public function __construct() {
		...
		/* Include pro support files/functions */
		include_once TIMELINE_EXPRESS_PATH . 'lib/support-files/support-functions.php';
		...
		/* Include the Filter Widget */
		include_once TIMELINE_EXPRESS_PATH . 'lib/classes/class-timeline-express-filter-widget.php';
	...
	public static function timeline_express_save_options( $options ) {
		...
		if ( ! isset( $_POST['timeline_express_settings_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['timeline_express_settings_nonce'] ) ), 'timeline_express_save_settings' ) ) {
			...
			wp_die(
				esc_html__( 'Sorry, the nonce security check did not pass. Please go back to the settings page, refresh the page and try to save your settings again.', 'timeline-express-pro' ),
				__( 'Failed Nonce Security Check', 'timeline-express-pro' ),
				array(
					'response'       => 500,
					'back_link'      => true,
		...
		$timeline_express_options['excerpt-random-length']              = (bool) isset( $options['excerpt-random-length'] ) ? $options['excerpt-random-length'] : false;
		...
		$timeline_express_options['pagination-bg-color']         = sanitize_text_field( $options['pagination-bg-color'] );
		$timeline_express_options['pagination-text-color']       = sanitize_text_field( $options['pagination-text-color'] );
		$timeline_express_options['pagination-hover-bg-color']   = sanitize_text_field( $options['pagination-hover-bg-color'] );
		$timeline_express_options['pagination-hover-text-color'] = sanitize_text_field( $options['pagination-hover-text-color'] );
		...
		$timeline_express_options['disable-animation']                           = (bool) isset( $options['disable-animation'] ) ? $options['disable-animation'] : false;
		$timeline_express_options['legacy_support']                              = (bool) isset( $options['legacy_support'] ) ? $options['legacy_support'] : false;
		$timeline_express_options['enable_comments']                             = (bool) isset( $options['enable_comments'] ) ? $options['enable_comments'] : false;
		$timeline_express_options['delete-announcement-posts-on-uninstallation'] = (bool) isset( $options['delete-announcement-posts-on-uninstallation'] ) ? $options['delete-announcement-posts-on-uninstallation'] : false;

		// Timeline Sidebar
		$timeline_express_options['timeline_sidebar'] = (bool) sanitize_text_field( $options['timeline_sidebar'] );
