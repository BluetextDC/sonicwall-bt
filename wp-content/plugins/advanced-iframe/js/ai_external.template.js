      ...
      if (ia_resize_init_done_PARAM_ID === false) {
        ...
        if (res_element.length === 0) {
            ...
            res_element.resize(function(){
                ...
                aiExecuteWorkaround_PARAM_ID(false);
            });
        }
 ...
 */
function aiExecuteWorkaround_PARAM_ID(init) {
    ...
    if (window!==window.top) { /* I'm in a frame! */
      ...
      if (updateIframeHeight === 'true') {
        ...
        if (!usePostMessage) {
          url = domain_PARAM_ID + '/js/iframe_height.html';
       ...
       if (onload_resize_delay === 0) {
        ...
        if (enable_responsive_iframe === 'true' && init) {
            ...
            window.setTimeout(initResize_PARAM_ID, onload_resize_delay + 1000);
        }
        if (showItNow) {
...
if (doIt) {
  ...
  if (typeof modify_iframe_if_cookie === 'undefined') {
      var modify_iframe_if_cookie = PARAM_MODIFY_IFRAME_IF_COOKIE;
  }
  ...
  if (typeof add_css_class_iframe === 'undefined') {
   ...
   if (typeof send_console_log === 'undefined') {
      var send_console_log = PARAM_SEND_CONSOLE_LOG;
  }
