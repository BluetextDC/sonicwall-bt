<?php 
    ...
    // $plc_data2 = json_encode($plc_request->get_plc_tables());
    // echo '--------------------------------plc_data:  ' . var_dump(stripslashes($plc_data2));
	...
	foreach ($plc_data->records as $prec) {
		...
		foreach ($p_name_array as $pnn) {
            ...
            // var_dump(json_encode($master_array));        // "{"Sonicwall GMS":{"software":[{"version":"GMS 8.6", "fsaof": "2019"}], [], [] }}"
...
<?php
    ...
    function software_sort($a, $b) {
        return strnatcasecmp($b['version'], $a['version']);
    }
    function firmware_sort($a, $b) {
        return strnatcasecmp($b['release'], $a['release']);
    }
    function hardware_sort($a, $b) {
		return strnatcasecmp($b['sortid'], $a['sortid']);
		// return strnatcasecmp($b['last_modified_time'], $a['last_modified_time']);
    }
    

    /*
    function renderPlcView($product_name, $product_types, $table_data) {
        $pn = str_replace('  ', ' ', $product_name);
        $pn_id = str_replace( '(', '', str_replace( ')', '', str_replace(' ', '-', strtolower( trim( $pn ) ) ) ) );
        $prod_title .= '<p id="' . str_replace(' ', '-', strtolower( trim( $pn ) ) ) . '" class="plc-product-name">' . $pn . '</p>';   // <p id="sonicwall-gms" class="plc-product-name">SonicWall GMS</p> 
        $prod_title .= '</div>';

        foreach ($product_types as $pt) {
            $types .= '<div class="plc-product-type ' . $pt . ' ' . $pn_id . '"><a class="button" href="#" id="' . $pn_id . '">' . $pt . '</a></div>';
        }
        $types .= '</div>';
    ...
    */
	...
	foreach ($master_array as $mk => $mv) {     // master_array => sonicwall GMS
		$pt = str_replace('  ', ' ', $mk);
		$pt_id = str_replace( '(', '', str_replace( ')', '', str_replace(' ', '-', strtolower( trim( $pt ) ) ) ) );     // sonicwall-gms
        $prod_title .= '<p id="' . str_replace(' ', '-', strtolower( trim( $pt ) ) ) . '" class="plc-product-name">' . $pt . '</p>';   // <p id="sonicwall-gms" class="plc-product-name">SonicWall GMS</p> 

		foreach ($mv as $kk => $kv) {   // "sonicwall GMS" as  "sonicwall GMS" => "software"
			if ( $kk == 'Software') {
                ...
                // usort($a,"order_by_weight");

                usort($kv, "software_sort");
                foreach ($kv as $pr) {  // sort 
					$tables .= '<tr>';
					foreach ($pr as $pd) {  // version as version
						$tables .= '<td>' . $pd . '</td>';
					}
					$tables .= '</tr>';
				}
